...
@push('styles')
  ...
  <style>
    ...
    @media (max-width: 768px) {
      ...
      /* Layout vertical pentru prețuri pe mobil */
      .price-layout-mobile {
        padding: 1rem 0;
      }

      .price-layout-mobile .h5 {
        font-size: 1.1rem !important;
      }

      .price-layout-mobile .h3 {
        font-size: 1.75rem !important;
      }
        ...
        <div class="d-flex align-items-center gap-3 flex-wrap price-header">
          @if($vehicle->has_active_offer)
            ...
            <div class="price-card-offer bg-danger-subtle border-0 p-3 rounded-4">
              <!-- Badge Descuento - FIRST on mobile -->
              @if($vehicle->discount_percentage)
              <div class="text-center text-md-start mb-3 mb-md-0 d-md-inline-block">
                ...
                <small class="text-danger d-block d-md-inline-block ms-md-2 mt-2 mt-md-0">Expira {{ \Carbon\Carbon::parse($vehicle->offer_expires_at)->format('d.m.Y') }}</small>
                @endif
              </div>
              @endif

              <!-- Precios - Vertical en móvil, horizontal en desktop -->
              <div class="price-layout-mobile d-md-none">
                <!-- Precio Original - móvil -->
                <div class="text-center mb-3">
                  <div class="text-muted small mb-1">Precio original</div>
                  <div class="h5 text-muted text-decoration-line-through mb-0">€ {{ number_format($extractPrice($baseOriginal)) }}</div>
                </div>

                <!-- Precio de Oferta - móvil -->
                <div class="text-center">
                  <div class="text-danger small mb-1">Precio de oferta</div>
                  <div class="h3 text-danger fw-bold mb-0">€ {{ number_format($extractPrice($vehicle->offer_price)) }}</div>
                </div>
              </div>

              <!-- Precios - Desktop -->
              <div class="d-none d-md-flex align-items-center gap-4 flex-wrap">
                <div>
                  <div class="text-muted small mb-1">Precio original</div>
          ...
          <table class="table table-bordered align-middle">
            <tbody>
              ...
              @if(!empty($vehicle->vin))
                <tr>
                  <th>Combustible</th><td>{{ $vehicle->fuel ?? 'N/A' }}</td>
                  <th>Etiqueta medioambiental</th><td><code>{{ substr($vehicle->vin,0,6) }}••••••</code></td>
                </tr>
              @else
                <tr>
                  <th>Combustible</th><td colspan="3">{{ $vehicle->fuel ?? 'N/A' }}</td>
                </tr>
              @endif
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Swipe navigation (mobile) on main media
  ...
  function onTouchEnd(e){
    ...
    if (Math.abs(dx) > 50 && dt < 1000) {
      if (dx < 0) { setCurrent(currentIndex + 1); } else { setCurrent(currentIndex - 1); }
      swipedRecently = true;
      setTimeout(function(){ swipedRecently = false; }, 250);
    }
    isSwiping = false;
  }
  if (mainMediaContainer) {
    mainMediaContainer.addEventListener('touchstart', onTouchStart, { passive: true });
    mainMediaContainer.addEventListener('touchmove', onTouchMove, { passive: true });
    mainMediaContainer.addEventListener('touchend', onTouchEnd, { passive: true });
    // Mouse drag support (desktop/tablet)
    let mouseDown = false, mouseStartX = 0, mouseStartY = 0, mouseStartTime = 0;
    mainMediaContainer.addEventListener('mousedown', function(e){
      mouseDown = true; mouseStartX = e.clientX; mouseStartY = e.clientY; mouseStartTime = Date.now();
    });
    ...
    mainMediaContainer.addEventListener('mouseup', function(e){
      if (!mouseDown) return; mouseDown = false;
      if (!isSwiping) return; isSwiping = false;
      const dx = e.clientX - mouseStartX; const dt = Date.now() - mouseStartTime;
      if (Math.abs(dx) > 50 && dt < 1000) {
        if (dx < 0) { setCurrent(currentIndex + 1); } else { setCurrent(currentIndex - 1); }
        swipedRecently = true; setTimeout(function(){ swipedRecently = false; }, 250);
      }
    });
  }
  ...
  if (mainMediaClick) {
    mainMediaClick.addEventListener('click', function(e) {
      ...
      // Nu deschide dacă tocmai a fost un swipe
      if (typeof swipedRecently !== 'undefined' && swipedRecently) {
  ...
  if (lightbox) {
    ...
    // Swipe on lightbox (touch) - Improved for mobile with dead zone for close button
    ...
    lightbox.addEventListener('touchstart', function(e) {
      ...
      // Check if touch started on close button element
      touchedCloseButton = e.target.closest('.lightbox-close') !== null;

      // Also check if touch is in the close button zone (top-right corner 100x100px)
      const isInCloseZone = t.clientX > window.innerWidth - 100 && t.clientY < 100;

      if (touchedCloseButton || isInCloseZone) {
        // Don't start swipe if touching close button or its zone
        touchedCloseButton = true;
        return;
      }
    ...
    lightbox.addEventListener('mousemove', function(e){ if(!lbMouseDown) return; const dx=e.clientX-lbMouseX; const dy=e.clientY-lbMouseY; if(!lbMouseSwiping && Math.abs(dx)>10 && Math.abs(dx)>Math.abs(dy)) lbMouseSwiping=true; });
    lightbox.addEventListener('mouseup', function(e){ if(!lbMouseDown) return; lbMouseDown=false; if(!lbMouseSwiping) return; lbMouseSwiping=false; const dx=e.clientX-lbMouseX; const dt=Date.now()-lbMouseTime; if (Math.abs(dx)>50 && dt<1000) { if (dx<0) { lightboxNextImage(); } else { lightboxPrevious(); } } });
  }
